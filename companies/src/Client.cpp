...
void Client::save(int client_number, std::string preffix) {
  try {
    std::string file_name =
        preffix + "_client_" + std::to_string(client_number) + ".txt";
    std::ofstream file{file_name};

    if (!file.is_open()) {
      throw(std::string("Impossible to create client file"));
    }
